function eliminateTinyWeights(weights) {
    const kept = weights.filter(weight => weight.weight >= 0.05);
    if (kept.length === 0)
        return [];
    let sum = 0;
    for (const weight of kept)
        sum += weight.weight;
    if (sum <= 0)
        return [];
...
function parseFaceCorner(value) {
...
export function parseMesh(data) {
    const vertices = [];
    const uvs = [];
    const normals = [];
    const meshes = new Map();
    const armatures = new Map();
    let mesh = null;
    let armature = null;
    let pendingWeights = [];
    let boneIndex = 0;
...
export function mergeBobjMesh(base, other) {
    ...
    for (const [name, armature] of other.armatures) {
        if (!armatures.has(name))
            armatures.set(name, armature);
    }
